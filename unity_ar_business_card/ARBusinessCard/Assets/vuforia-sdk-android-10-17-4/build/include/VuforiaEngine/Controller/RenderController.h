...
typedef struct VuVideoBackgroundViewInfo
{
    ...
    /// \note This is identical to the values that can be retrieved from \ref vuRenderControllerGetVideoBackgroundViewport
    VuVector4I viewport;
 ...
 *
 * \note The render view configuration is used in the calculation of the projection matrix reported in the \ref VuRenderState that is
 * intended for augmentation rendering (field \ref VuRenderState#projectionMatrix). Other values used in this calculation of the projection
 * matrix are the near and far planes set in \ref vuRenderControllerSetProjectionMatrixNearFar and the view orientation set in \ref
 * vuPlatformControllerSetViewOrientation.
 *
 * \note The render view configuration is also used in the calcualtion of the viewport if no custom viewport has been set. See \ref
 * vuRenderControllerGetVideoBackgroundViewport for details.
 *
 * \note Several API calls depend on the availability of a render view configuration (either default one or user provided). If no render
 * view configuration is available the calls to such as \ref vuRenderControllerSetVideoBackgroundViewport, \ref
 * vuRenderControllerGetVideoBackgroundViewport, \ref vuRenderControllerSetVideoBackgroundViewportMode and \ref
 * vuRenderControllerGetVideoBackgroundViewInfo will fail and the call to \ref vuPlatformControllerSetViewOrientation might not be applied
 * correctly.
 *
 ...
 *
 * If no viewport is available at all because Vuforia Engine is not running and no viewport has been set via \ref
 ...
 * for augmentation rendering (field \ref VuRenderState#projectionMatrix). Other values used in this calculation of the projection matrix
 * are the near and far planes set in \ref vuRenderControllerSetProjectionMatrixNearFar and the view orientation set in \ref
 * vuPlatformControllerSetViewOrientation.
 *
