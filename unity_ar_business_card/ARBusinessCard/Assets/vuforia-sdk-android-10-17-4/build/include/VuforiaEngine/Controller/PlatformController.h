 ...
 * below.
 *
 * \note On Android, Vuforia does internal view orientation consistency checking. Therefore calls to this API will fail on this platform
 * if the provided view orientation does not align with the actual view orientation.
 *
 * \note Updates to the view orientation depend on the availability of a render view configuration in Engine. On platforms where no default
 * render view configuration is available \ref vuRenderControllerSetRenderViewConfig should be called immediately after Engine has been
 * created and prior to calling \ref vuPlatformControllerSetViewOrientation. Otherwise the provided view orientation might not be applied.
 * Note that no failure will be reported on the API level in this case but only an error log might be emmited.
 *
 * \see vuRenderControllerSetRenderViewConfig for more details on the default render view configuration and the requirements regarding
 * updating the render view configuration.
 *
 * \note The view orientation is also used in the calculation of the projection matrix reported in the \ref VuRenderState that is intended
 * for augmentation rendering (field \ref VuRenderState#projectionMatrix). The orientation should be consistent with the width and height
 * set in \ref vuRenderControllerSetRenderViewConfig.
 *
